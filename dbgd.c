...
static int dbgd_mem_read(Dbg__Request *req, Dbg__Response *res)
{
    ...
    while((s > 0) && (s % 4 == 0)) {
      *(uint32_t*)&res->buffer.data[i] = *(uint32_t*)(req->address + i);
      i += 4;
      s -= 4;
    }
    while((s > 0) && (s % 2 == 0)) {
      *(uint16_t*)&res->buffer.data[i] = *(uint16_t*)(req->address + i);
      i += 2;
      s -= 2;
    }
    while(s > 0) {
      *(uint8_t*)&res->buffer.data[i] = *(uint8_t*)(req->address + i);
      i += 1;
      s -= 1;
    }
...
static int dbgd_mem_write(Dbg__Request *req, Dbg__Response *res)
{
    if (!req->has_address || !req->has_buffer)
        return DBG__RESPONSE__TYPE__ERROR_INCOMPLETE_REQUEST;
    ...
    while((s > 0) && (s % 4 == 0)) {
      *(uint32_t*)(req->address + i) = *(uint32_t*)&res->buffer.data[i];
      i += 4;
      s -= 4;
    }
    while((s > 0) && (s % 2 == 0)) {
      *(uint16_t*)(req->address + i) = *(uint16_t*)&res->buffer.data[i];
      i += 2;
      s -= 2;
    }
    while(s > 0) {
      *(uint8_t*)(req->address + i) = *(uint8_t*)&res->buffer.data[i];
      i += 1;
      s -= 1;
    }
